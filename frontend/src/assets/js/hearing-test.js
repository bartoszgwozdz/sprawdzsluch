...
class HearingTest {
  ...
  /**
   * Generuje i wyświetla wyniki testu w podanym kontenerze.
   * @param {string} containerId ID elementu, w którym mają zostać wyświetlone wyniki.
   */
  displayResults(containerId) {
    ...
    this.hearingLevels.forEach(item => resultsMap.set(item.frequency, item.gain));
    const maxGainEntry = [...this.hearingLevels.entries()].reduce((a ,b) => a.gain > b.gain ? a : b);

    console.log("displayResults - resultsMap:", resultsMap); // ADDED LOG
    const oneKhzResult = resultsMap.get(1000);
    const referenceGain = oneKhzResult || this.referenceLevel;

    this.frequencyOrder.push(1000); // Ensure 1kHz is always included in the frequencies
    const sortedFrequencies = this.frequencyOrder.sort((a, b) => a - b);


    let tableHeader = '<th>Frequency</th>';
    let tableBody = '<td><strong>Level (dB)</strong></td>';
    let minDb = 0, maxDb = 0;

    // 2. Przetwórz wyniki i zbuduj tabelę
    sortedFrequencies.forEach(freq => {
      const gain = resultsMap.get(freq);
      tableHeader += `<th>${freq < 1000 ? freq + ' Hz' : (freq / 1000) + ' kHz'}</th>`;

      if (gain && referenceGain > 0) {
        const rawDbValue = 20 * Math.log10(referenceGain/gain);

        // Zastosuj korekcję krzywej izofonicznej
        const correctionFactor = this.getIsofonicCorrection(freq);
        const correctedDbValue = rawDbValue + correctionFactor;


        if (correctedDbValue < minDb) minDb = correctedDbValue;
        if (correctedDbValue > maxDb) maxDb = correctedDbValue;

        let bgColor = '#ccffcc'; // Zielony (dobry słuch)
        if (correctedDbValue < -20) bgColor = '#ffcccc'; // Czerwony (gorszy słuch)
        else if (correctedDbValue < -10) bgColor = '#ffffcc'; // Żółty (lekki niedosłuch)

        tableBody += `<td style="background-color: ${bgColor};">${correctedDbValue.toFixed(1)} dB</td>`;
      } else {
        tableBody += `<td>N/A</td>`;
      }
    });
    ...
    const lowestFreq = sortedFrequencies[0] || 0;
    const highestFreq = this.maxAudibleFrequency || 0;
    const normalRange = this.age < 20 ? "20Hz-20kHz" : this.age < 40 ? "20Hz-17kHz" : this.age < 60 ? "20Hz-15kHz" : "20Hz-12kHz";

    // 4. Zbuduj cały HTML i wstaw go do kontenera
    resultsContainer.innerHTML = `
      ...
      <div class="results-summary">
        ...
        <p>Lowest tested frequency: <strong>${lowestFreq} Hz</strong></p>
        <p>Highest audible frequency: <strong>${highestFreq} Hz</strong></p>
        <p>Normal range for your age (${this.age}): <strong>${normalRange}</strong></p>
